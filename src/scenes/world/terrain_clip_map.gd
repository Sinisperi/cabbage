...
func _ready() -> void:
	...
	(lod_0_col.shape as HeightMapShape3D).update_map_data_from_image(collision_map_image, 0.0, 30.0)
...
func update_collision(spacing: float) -> void:
	...
	var world_to_pixel: float = WORLD_SIZE / 2048.0
	var limit: float = WORLD_SIZE / 2.0 - (collision_map_size.x / 2.0)
	pos.x = floor(camera.global_position.x / world_to_pixel) * world_to_pixel + 0.5
	...
	if pos.x != old_col_pos.x || pos.y != old_col_pos.y:
		...
		#var uv_x: float = (pos.x + (WORLD_SIZE / 2.0)) / WORLD_SIZE
		#var uv_y: float = (pos.y + (WORLD_SIZE / 2.0)) / WORLD_SIZE
		#var center_x: float = uv_x * 2048
		#var center_y: float = uv_y * 2048
		#var tx: float = floor(center_x - (collision_map_size.x / 2.0))
		#var ty: float = floor(center_y - (collision_map_size.y / 2.0))
		...
		temp_img.resize(int(collision_map_size.x / 4.0), int(collision_map_size.x / 4.0), Image.INTERPOLATE_NEAREST)
		lod_0_col.scale = Vector3.ONE * 4.0
		(lod_0_col.shape as HeightMapShape3D).update_map_data_from_image(temp_img, 0.0, 30.0 / 4.0)
		old_col_pos = pos
		...
		#lod_0_col.global_position.x = pos.x - world_offset
		#lod_0_col.global_position.z = pos.y - world_offset
